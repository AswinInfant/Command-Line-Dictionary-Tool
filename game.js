...
function synRemove(guesswordRemove) {
  for (i = 0; i < synonyms.length; i++) {
    if(synonyms[i].localeCompare(guesswordRemove) == 0) {
      synonyms.splice(i,1);
    }
  }
} 

function synCompare(guesswordCompare) {
  var isPresent = synonyms.includes(guesswordCompare);
  if(isPresent == 1) {
    synRemove(guesswordCompare);
    return isPresent; 
  }
  else {
    return isPresent;
